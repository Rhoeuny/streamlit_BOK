...
st.write("논문 구현의 최종 결과로 동일 기간동안에 의사록 문서의 톤과 기준금리의 상관관계를 나타내는 페이지")

st.subheader("2. 데이터를 기반으로 제작한 극성사전")
st.write("뉴스, 채권보고서, 의사록을 통해 제작한 Dovish,Hawkish 사전 별 상위 10개 단어와 그 단어가 포함된 의사록 문장을 보여주는 페이지")

st.subheader("3. 선택한 날짜의 뉴스 크롤링")
st.write("선택한 날짜에 '금리' 키워드로 검색한 네이버 뉴스 크롤링 페이지")

st.subheader("4. 의사록 문장톤을 확인하는 챗봇")
